  ...
  const parseMarkdown = (text: string) => {
    ...
    for (let i = 0; i < parts.length; i++) {
      ...
      if (codeIndex < codeMatches.length) {
        ...
        elements.push(
          <div key={`code-${i}`} className="bg-[#0F0F12] p-4 mb-4 shadow-lg relative">
            <button
              ...
              title="Copy code"
            >
              <span className="material-symbols-outlined text-[16px] text-white/60 group-hover:text-orange-500">
                {copiedCode === codeContent ? 'check' : 'content_copy'}
              </span>
            </button>
            <pre className="code-block text-[13px] text-orange-500 overflow-x-auto pr-12">
    ...
    for (let i = 0; i < parts.length; i++) {
      if (i % 2 === 1) {
        ...
        elements.push(
          <code 
            key={`inline-${key}-${i}`} 
            className="bg-[#1C1B1E] px-1 py-0.5 rounded text-[13px] text-orange-500 font-mono"
          >
            {parts[i]}
          </code>
        )
